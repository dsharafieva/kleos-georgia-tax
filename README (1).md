...
## ICP-alignment edits (post-audit)

An honest audit found the "Keeping the 1%" section and the cases grid had
drifted toward the contractor's own tax-optimization concerns rather than the
hiring company's (Kleos's actual buyer). Four fixes went in:

1. **Checklist rebuilt to only include buyer-observable signals.** The old
   6-item list mixed things a hiring company can actually see about its own
   arrangement (single client, fixed fee, integration) with things only the
   contractor would know (their total turnover across *all* clients, their
   own filing history, their personal rental/dividend income). The new
   checklist keeps only the former — fixed hours, dominant client, team
   integration, exclusivity, SOW-scoping into an excluded activity, no
   independent business — mirroring the discipline of the Belarus template's
   misclassification checklist. The three contractor-only facts (turnover
   ceiling, missed filings, non-business income) moved to a separate,
   non-tickable "Also worth asking — but only your contractor can answer
   these" note (`.ask` block, question-mark bullets, not checkboxes) so nothing
   was deleted, just correctly attributed.
2. **Section reframed around the buyer's exposure, not the contractor's rate.**
   Headline changed from "The 1% rate is a status, not a right" to "One
   invoice away from being an employer" (nav label: Misclassification, not
   "Keeping the 1%"). The sub-copy and all three verdict tiers now state the
   consequence in terms of *your* compliance question, not just whether the
   contractor keeps 1%.
3. **Cases grid trimmed to the 3 buyer-actionable cases**, each re-labelled
   with what the buyer actually does about it (sets the rate / you control
   this / confirm before you hire). The other 3 (source-of-income mechanics,
   the 500k ceiling, the VAT threshold) were kept — the facts are still
   correct and useful context — but demoted to a compact, un-carded
   "Also moves the number, but it's the contractor's mechanic" list so the
   page doesn't imply the buyer should act on them.
4. **One explicit Kleos-mechanism line added near the CTA**: "We check the
   activity code, keep the monthly filings current, and confirm the
   labour-permit exemption before the first invoice goes out — so 'still
   qualifies' isn't something you find out the hard way." This ties the
   page's own depth back to the reason to use Kleos instead of DIY-ing the
   compliance surface just described.
